...
import { AlertCircle, FileCode, FileText, Loader2, UploadCloud, X } from 'lucide-react'
...
export function DocumentUploadZone({
  ...
  const validateFiles = (files: File[]): { valid: File[]; error: string | null } => {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        return {
          valid: [],
          error: `文件「${file.name}」(${formatBytes(file.size)}) 超过了 50MB 单文件上限`,
        }
      }
  ...
  const handleUpload = async (files: File[]) => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : '上传失败，请检查网络或稍后重试')
    } finally {
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              支持{' '}
              <span className="font-medium text-zinc-700 dark:text-zinc-300">Markdown (.md)</span>、
