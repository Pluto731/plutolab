...
export default function KnowledgeBaseDetailPage() {
  ...
  const {
    ...
    refetchInterval: (query) => {
      ...
      const hasProcessing = docs.some((d) => d.status === 'pending' || d.status === 'parsing')
  ...
  useEffect(() => {
    const hasProcessing = documents.some((d) => d.status === 'pending' || d.status === 'parsing')
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-16 text-center">
        <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-8">
          <h2 className="text-base font-semibold text-destructive">知识库未找到或加载失败</h2>
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      {/* KB Header */}
      <KBHeader kb={kb} onOpenImportModal={() => setImportModalOpen(true)} />

      {/* Document Upload Zone */}
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">上传资料文件</h3>
        <DocumentUploadZone kbId={kbId} onUploadSuccess={handleUploadSuccess} />
      </div>

      {/* Document Real-Time Status Table */}
      <ErrorNotice message={docsError?.message ?? deleteMutation.error?.message ?? null} />
      <DocumentTable
        documents={documents}
        isLoading={docsLoading}
